...
Copyright (c) 2011 - 2015, dmazar. All rights reserved.<BR>
...
//
ESAL_VARIABLE_GLOBAL  *gVariableModuleGlobal;
...
)
{
  ...
  if (Iso639Language) {
    ...
    //
    CompareLength = ISO_639_2_ENTRY_SIZE;
    gVariableModuleGlobal->Lang[CompareLength] = '\0';
    return CopyMem (gVariableModuleGlobal->Lang, SupportedLang + Index * CompareLength, CompareLength);
    ...
    while (TRUE) {
      ...
      if (SubIndex == Index) {
        ...
        // In driver entry, it pre-allocates a runtime attribute memory to accommodate this string.
        //
        gVariableModuleGlobal->PlatformLang[CompareLength] = '\0';
        return CopyMem(gVariableModuleGlobal->PlatformLang, Supported - CompareLength, CompareLength);
      }
      SubIndex++;
    ...
    while (LanguageLength > 0) {
      ...
      for (Supported = SupportedLanguages; *Supported != '\0'; Supported += CompareLength) {
        ...
        if (AsciiStrnCmp (Supported, Language, LanguageLength) == 0) {
          ...
          Buffer = Iso639Language ? gVariableModuleGlobal->Lang : gVariableModuleGlobal->PlatformLang;
  ...
  )
{
  ...
  if (StrCmp (VariableName, L"PlatformLangCodes") == 0) {
    ...
    //
    if (gVariableModuleGlobal->PlatformLangCodes != NULL) {
      FreePool (gVariableModuleGlobal->PlatformLangCodes);
    }
    gVariableModuleGlobal->PlatformLangCodes = AllocateRuntimeCopyPool (DataSize, Data);
    ASSERT (gVariableModuleGlobal->PlatformLangCodes != NULL);

    //
    // PlatformLang holds a single language from PlatformLangCodes, 
    // so the size of PlatformLangCodes is enough for the PlatformLang.
    //
    if (gVariableModuleGlobal->PlatformLang != NULL) {
      FreePool (gVariableModuleGlobal->PlatformLang);
    }
    gVariableModuleGlobal->PlatformLang = AllocateRuntimePool (DataSize);
    ASSERT (gVariableModuleGlobal->PlatformLang != NULL);
    ...
    // Therefore, in variable driver, only store the original value for other use.
    //
    if (gVariableModuleGlobal->LangCodes != NULL) {
      FreePool (gVariableModuleGlobal->LangCodes);
    }
    gVariableModuleGlobal->LangCodes = AllocateRuntimeCopyPool (DataSize, Data);
    ASSERT (gVariableModuleGlobal->LangCodes != NULL);
  }

  if (SetLanguageCodes 
      && (gVariableModuleGlobal->PlatformLangCodes != NULL)
      && (gVariableModuleGlobal->LangCodes != NULL)) {
    //
    // Update Lang if PlatformLang is already set
    // Update PlatformLang if Lang is already set
    //
    Status = FindVariable (L"PlatformLang", &gEfiGlobalVariableGuid, &Variable, (VARIABLE_GLOBAL *) gVariableModuleGlobal);
    if (!EFI_ERROR (Status)) {
      //
      // Update Lang
      //
      ...
      Status = FindVariable (L"Lang", &gEfiGlobalVariableGuid, &Variable, (VARIABLE_GLOBAL *) gVariableModuleGlobal);
      if (!EFI_ERROR (Status)) {
        //
        // Update PlatformLang
        //
  ...
  if (StrCmp (VariableName, L"PlatformLang") == 0) {
    ...
    //
    if ((gVariableModuleGlobal->PlatformLangCodes != NULL) && (gVariableModuleGlobal->LangCodes != NULL)) {
      //
      // When setting PlatformLang, firstly get most matched language string from supported language codes.
      //
      BestPlatformLang = VariableGetBestLanguage (gVariableModuleGlobal->PlatformLangCodes, FALSE, Data, NULL);
      if (BestPlatformLang != NULL) {
        //
        // Get the corresponding index in language codes.
        //
        Index = GetIndexFromSupportedLangCodes (gVariableModuleGlobal->PlatformLangCodes, BestPlatformLang, FALSE);

        //
        // Get the corresponding ISO639 language tag according to RFC4646 language tag.
        //
        BestLang = GetLangFromSupportedLangCodes (gVariableModuleGlobal->LangCodes, Index, TRUE);

        //
        // Successfully convert PlatformLang to Lang, and set the BestLang value into Lang variable simultaneously.
        //
        FindVariable (L"Lang", &gEfiGlobalVariableGuid, &Variable, (VARIABLE_GLOBAL *)gVariableModuleGlobal);
    ...
    if ((gVariableModuleGlobal->PlatformLangCodes != NULL) && (gVariableModuleGlobal->LangCodes != NULL)) {
      ...
      //
      BestLang = VariableGetBestLanguage (gVariableModuleGlobal->LangCodes, TRUE, Data, NULL);
      if (BestLang != NULL) {
        //
        // Get the corresponding index in language codes.
        //
        Index = GetIndexFromSupportedLangCodes (gVariableModuleGlobal->LangCodes, BestLang, TRUE);

        //
        // Get the corresponding RFC4646 language tag according to ISO639 language tag.
        //
        BestPlatformLang = GetLangFromSupportedLangCodes (gVariableModuleGlobal->PlatformLangCodes, Index, FALSE);

        //
        // Successfully convert Lang to PlatformLang, and set the BestPlatformLang value into PlatformLang variable simultaneously.
        //
        FindVariable (L"PlatformLang", &gEfiGlobalVariableGuid, &Variable, (VARIABLE_GLOBAL *)gVariableModuleGlobal);
  ...
  )
{
  ...
  Global = &gVariableModuleGlobal->VariableGlobal[Physical];
  ...
  if ((Attributes & EFI_VARIABLE_NON_VOLATILE) != 0) {
    ...
    if ((((Attributes & EFI_VARIABLE_HARDWARE_ERROR_RECORD) != 0) 
      && ((HEADER_ALIGN (VarSize) + gVariableModuleGlobal->HwErrVariableTotalSize) > PcdGet32 (PcdHwErrStorageSize)))
      || (((Attributes & EFI_VARIABLE_HARDWARE_ERROR_RECORD) == 0) 
      && ((HEADER_ALIGN (VarSize) + gVariableModuleGlobal->CommonVariableTotalSize) > NonVolatileVarableStoreSize - sizeof (VARIABLE_STORE_HEADER) - PcdGet32 (PcdHwErrStorageSize)))) {
      Status = EFI_OUT_OF_RESOURCES;
      goto Done;
    }

    NextVariable  = (VARIABLE_HEADER *) (UINT8 *) (gVariableModuleGlobal->NonVolatileLastVariableOffset
                      + (UINTN) Global->NonVolatileVariableBase);
    gVariableModuleGlobal->NonVolatileLastVariableOffset += HEADER_ALIGN (VarSize);

    if ((Attributes & EFI_VARIABLE_HARDWARE_ERROR_RECORD) != 0) {
      gVariableModuleGlobal->HwErrVariableTotalSize += HEADER_ALIGN (VarSize);
    } else {
      gVariableModuleGlobal->CommonVariableTotalSize += HEADER_ALIGN (VarSize);
    }
  } else {
    if ((UINT32) (HEADER_ALIGN (VarSize) + gVariableModuleGlobal->VolatileLastVariableOffset) >
    ...
    NextVariable    = (VARIABLE_HEADER *) (UINT8 *) (gVariableModuleGlobal->VolatileLastVariableOffset
                        + (UINTN) Global->VolatileVariableBase);
    gVariableModuleGlobal->VolatileLastVariableOffset += HEADER_ALIGN (VarSize);
  }
  ...
  )
{
  ...
  //
  // No support for authenticated variable write yet.
  //
  ...
  )
{
  ...
  if (VolatileStore) {
    VariableBase = &gVariableModuleGlobal->VariableGlobal[Physical].VolatileVariableBase;
    LastVariableOffset = &gVariableModuleGlobal->VolatileLastVariableOffset;
  } else {
    VariableBase = &gVariableModuleGlobal->VariableGlobal[Physical].NonVolatileVariableBase;
    LastVariableOffset = &gVariableModuleGlobal->NonVolatileLastVariableOffset;
  }
  ...
  )
{
  ...
  //
  // Allocate memory for gVariableModuleGlobal
  //
  gVariableModuleGlobal = (ESAL_VARIABLE_GLOBAL *) AllocateRuntimeZeroPool (
                                                    sizeof (ESAL_VARIABLE_GLOBAL)
                                                   );
  if (NULL == gVariableModuleGlobal) {
    return EFI_OUT_OF_RESOURCES;
  }

  EfiInitializeLock(&gVariableModuleGlobal->VariableGlobal[Physical].VariableServicesLock, TPL_NOTIFY);
  ...
  if (EFI_ERROR (Status)) {
    FreePool(gVariableModuleGlobal);
    return Status;
  }
  //
